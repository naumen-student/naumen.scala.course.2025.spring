object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    def factorize(n: Int, factor: Int, factors: Set[Int]): Set[Int] = {
      if (n <= 1) factors
      else if (n % factor == 0) factorize(n / factor, factor, factors + factor)
      else factorize(n, factor + 1, factors)
    }

    factorize(Math.abs(number), 2, Set.empty).toSeq.sorted
  }
