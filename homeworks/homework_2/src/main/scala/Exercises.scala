object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        if (numberCopy > 1) result += numberCopy
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val calculateWeight = (radius: Int, density: Double) => (4D / 3D) * Math.PI * Math.pow(radius, 3) * density

        ballsArray.toList.sortBy {case (_, (r, d)) => calculateWeight(r, d)}.map {case (n, _) => n}
    }
}