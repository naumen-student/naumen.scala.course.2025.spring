object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def findFactors(n: Int, divisor: Int, acc: Set[Int]): Set[Int] = {
            if (n <= 1) acc
            else if (n % divisor == 0) findFactors(n / divisor, divisor, acc + divisor)
            else findFactors(n, divisor + 1, acc)
        }
        
        findFactors(number, 2, Set()).toSeq.sorted
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def calculateWeight(radius: Int, density: Double): Double = {
            (4.0 / 3.0) * java.lang.Math.PI * math.pow(radius, 3) * density
        }
        
        ballsArray.toSeq
            .map { case (name, (radius, density)) => (name, calculateWeight(radius, density)) }
            .sortBy(_._2)
            .map(_._1)
    }
