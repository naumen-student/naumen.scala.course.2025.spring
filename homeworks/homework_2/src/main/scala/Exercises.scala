object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    def factorize(n: Int, divisor: Int, acc: Set[Int]): Set[Int] = {
    ...
    if (number > -2 && number < 2) Seq()
    else factorize(math.abs(number), 2, Set()).toSeq.sorted
  }
  ...
  ): Seq[String] = ballsArray.toSeq
    .sortBy { case (name, (radius, density)) =>
