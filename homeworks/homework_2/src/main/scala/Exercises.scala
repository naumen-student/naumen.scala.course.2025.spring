object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
          ...
          var weightMap = Map.empty[String, Double]
          for ((k, v) <- ballsArray) {
              weightMap += (k -> ((4 / 3) * Pi * v._1 * v._1) * v._2)
          }
          var resultSeq: Seq[String] = Seq.empty[String]
          var resultList : List[(String, Double)] = weightMap.toList.sortBy(_._2)
          for ((v1, v2) <- resultList) {
              resultSeq += v1
          }
          resultSeq
