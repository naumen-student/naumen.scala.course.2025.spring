...
//  def isPrime(multiplier: Int): Boolean = {
//    if (n == 2) {
//      true
//    } else if (n % 2 == 0) {
//      false
...
//  }
  ...
  def isPrime(n: Int): Boolean = {
    ...
    else (3 to math.sqrt(n).toInt by 2).forall(i => n % i != 0)
  }

  def findPrimeFactors(number: Int): Seq[Int] = {
    (2 to number).filter(i => number % i == 0 && isPrime(i))
  }

  def primeFactor(number: Int): Seq[Int] = {
    findPrimeFactors(number).distinct
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq
      .map { case (name, (radius, density)) => (name, (4.0 / 3.0) * java.lang.Math.PI * Math.pow(radius, 3) * density) }
