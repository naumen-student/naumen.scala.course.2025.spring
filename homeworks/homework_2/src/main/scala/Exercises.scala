object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factors(n: Int, a: Int = 2, acc: Set[Int] = Set()): Set[Int] = {
            if (n < 2) acc
            else if (n % a == 0) factors(n / a, a, acc + a)
            else factors(n, a + 1, acc)
        }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def calculateMass(radius: Int, density: Double): Double =
            (4.0 / 3.0) * scala.math.Pi * Math.pow(radius, 3) * density

        ballsArray.toSeq
          .sortBy { case (_, (radius, density)) => calculateMass(radius, density) }
          .map(_._1)
    }
}