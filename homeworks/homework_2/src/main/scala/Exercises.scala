...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        if (number <= 1 ) return Seq.empty

        @tailrec
        def calculateFactors(x: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (x <= 1) factors
            else if (x % divisor == 0) {
                calculateFactors(x / divisor, divisor, if(factors.contains(divisor)) factors else factors :+ divisor)
            } else {
                calculateFactors(x, divisor + 1, factors)
            }
        }

        calculateFactors(number, 2, Seq.empty)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortBy { case (_, size) => 4 / 3 * math.pow(size._1, 2) * math.Pi * size._2 }
          .map(_._1)
    }
