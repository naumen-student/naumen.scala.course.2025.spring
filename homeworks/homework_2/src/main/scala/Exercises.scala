...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factors(num: Int, divisor: Int): Seq[Int] = {
	      	 if (num <= 1) Seq.empty
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        val ballsWithMass = ballsArray.map { case (name, (radius, density)) =>
            (name, calculateMass(radius, density))}

        ballsWithMass.toSeq.sortBy(_._2).map(_._1)
    }
}