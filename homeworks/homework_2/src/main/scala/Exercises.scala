...
object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
    (iFrom to iTo).filter(i => i % 3 == 0 || i % 5 == 0).map(i => i.toLong).sum
  }
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def fact(num: Int, div: Int, stash: Seq[Int]): Seq[Int] = {
      if (num < div * div) {
        if (num > 1) {
          stash :+ num
        } else {
        ...
        fact(num / div, div, stash :+ div)
      } else {
        fact(num, div + 1, stash)
      }
    }
    ...
    fact(math.abs(number), 2, Seq()).distinct
  }
  ...
  def sumScalars(leftVector0: Vector2D, leftVector1: Vector2D, rightVector0: Vector2D, rightVector1: Vector2D): Double =
    sumByFunc(leftVector0, leftVector1, scalar, rightVector0, rightVector1)

  def sumCosines(leftVector0: Vector2D, leftVector1: Vector2D, rightVector0: Vector2D, rightVector1: Vector2D): Double =
    sumByFunc(leftVector0, leftVector1, cosBetween, rightVector0, rightVector1)

  private def sumByFunc(leftVector0: Vector2D, leftVector1: Vector2D,
                        func: (Vector2D, Vector2D) => Double,
                        rightVector0: Vector2D, rightVector1: Vector2D): Double =
    func(leftVector0, leftVector1) + func(rightVector0, rightVector1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def calcMass(r: Int, d: Double): Double = {
      (4.0 / 3.0) * scala.math.Pi * Math.pow(r, 3) * d
    }
    ballsArray.toSeq.sortBy { case (_, (r, d)) => calcMass(r, d) }.map(f => f._1)
  }
}