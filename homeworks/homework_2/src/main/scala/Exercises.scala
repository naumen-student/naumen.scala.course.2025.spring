object Exercises {
  ...
  */
  ...
  private def getMass(radius: Int, density: Double): Double =
    4 / 3 * java.lang.Math.PI * math.pow(radius, 3) * density

  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray
      .toSeq
      .sortBy { case (_, (radius, density)) => getMass(radius, density) }
      .map(_._1)
  }
}