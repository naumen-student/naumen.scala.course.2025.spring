object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def findUniqueFactors(num: Int, divisor: Int): Set[Int] = {
            if (num < 2) Set.empty[Int]
            else if (num % divisor == 0) {
                findUniqueFactors(num / divisor, divisor) + divisor
            }
            else if (divisor * divisor > num) {
                Set(num)
            }
            else {
                findUniqueFactors(num, divisor + 1)
            }
        }

        findUniqueFactors(number, 2).toSeq.sorted
    }
