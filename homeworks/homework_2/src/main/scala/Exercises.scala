...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def getFactors(n: Int, divisor: Int = 2, factors: Seq[Int] = Seq()): Seq[Int] = {
            if (n < 2) {
                factors
            }
            else if (n % divisor == 0) {
                getFactors(n / divisor, divisor, factors :+ divisor)
            }
            else {
                getFactors(n, divisor + 1, factors)
            }
        }
    ...
    private def sumByFunc(leftVector0: Vector2D,
                          leftVector1: Vector2D,
                          func: (Vector2D, Vector2D) => Double,
                          rightVector0: Vector2D,
                          rightVector1: Vector2D): Double =
        func(leftVector0, leftVector1) + func(rightVector0, rightVector1)

    def sumScalars(leftVector0: Vector2D,
                   leftVector1: Vector2D,
                   rightVector0: Vector2D,
                   rightVector1: Vector2D): Double =
        sumByFunc(leftVector0,
            leftVector1,
            scalar,
            rightVector0,
            rightVector1)

    def sumCosines(leftVector0: Vector2D,
                   leftVector1: Vector2D,
                   rightVector0: Vector2D,
                   rightVector1: Vector2D): Double =
        sumByFunc(leftVector0,
