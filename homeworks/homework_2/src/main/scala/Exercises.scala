object Exercises {
  ...
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  private def getFactors(n: Int, factor: Int): Seq[Int] = {
    if (factor * factor > n) if (n > 1) Seq(n) else Seq()
    else if (n % factor == 0) factor +: getFactors(n / factor, factor)
    else getFactors(n, factor + 1)
  }

  def primeFactor(number: Int): Seq[Int] =
    getFactors(number, 2).distinct
  ...
  private def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) =
  ...
  */
  ...
  private def computeMass(radius: Int, density: Double): Double =
    4.0 / 3.0 * math.Pi * math.pow(radius.toDouble, 3) * density

  // Возвращает список названий материалов, отсортированных по массе шариков в возрастании
  def sortMaterialsByMass(data: Map[String, (Int, Double)] = balls): Seq[String] =
    data.toSeq.sortBy { case (_, (r, dens)) => computeMass(r, dens) }.map(_._1)
}