...
object Exercises {
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): List[Int] = {
        def factors(n: Int, divisor: Int = 2, acc: Set[Int] = Set()): Set[Int] = {
            if (n < 2) acc
            else if (n % divisor == 0) factors(n / divisor, divisor, acc + divisor)
            else factors(n, divisor + 1, acc)
        }

        factors(number).toList.sorted
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortBy { case (_, size) => math.Pi * size._2 * 4 / 3 * math.pow(size._1, 2) }
          .map(_._1)
    }
}