...
object Exercises {
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                f: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = {
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def sorter(f: (Int, Double), s: (Int, Double)) = {
      math.pow(f._1, 3) * f._2 < math.pow(s._1, 3) * s._2
    }

    ballsArray.toSeq.sortWith((x, y) => sorter(x._2, y._2)).map(_._1)
  }
