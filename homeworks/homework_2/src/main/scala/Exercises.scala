object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def decompose(number: Int, divider: Int, divisorsSeq: Seq[Int]): Seq[Int] = {
            if (divider * divider > number) divisorsSeq :+ number
            else number % divider match {
                case 0 => decompose(number / divider, divider, divisorsSeq :+ divider)
                case _ => decompose(number, divider + 1, divisorsSeq)
            }
        }

        decompose(number, 2, Seq.empty[Int])
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        ballsArray.toList.sortBy { case (_, (radius, density)) => calculateMass(radius, density) }.map(_._1)
    }
