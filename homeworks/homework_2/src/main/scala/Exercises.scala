object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom.toLong to iTo.toLong).filter(x => x % 3 == 0 || x % 5 == 0).sum
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toArray.sortBy(metal => calculateWeight(metal._2._1, metal._2._2)).map(metal => metal._1)
    }

    def calculateWeight(r: Int, d: Double): Double = 4.0 / 3.0 * java.lang.Math.PI * math.pow(r, 3) * d
