object Exercises {
    ...
    private def isPrimeNumber(number: Int): Boolean = {
        !(2 to number).exists(i => number % i == 0 && number != i)
    }

    def primeFactor(number: Int): Seq[Int] = {
        (2 to number)
          .filter( i => isPrimeNumber(i) && number % i == 0)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.keys.toSeq.sortWith((first, second) => {
            val firstParams: (Int, Double) = ballsArray.getOrElse(first, (0,0))
            val secondParams: (Int, Double) = ballsArray.getOrElse(second, (0,0))
            val firstValue = (4.0 / 3) * Math.pow(firstParams._1, 3) * java.lang.Math.PI
            val secondValue = (4.0 / 3) * Math.pow(secondParams._1, 3) * java.lang.Math.PI
            (firstValue * firstParams._2) < (secondValue * secondParams._2)
        })
    }
