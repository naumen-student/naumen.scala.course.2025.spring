...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        // Calculate the sum of all numbers in the range divisible by 3 or 5
        var sum = 0
        for (i <- iFrom to iTo) {
            if (i % 3 == 0 || i % 5 == 0) {
                sum += i
    ...
    def primeFactor(number: Int): Seq[Int] = {
      if (number < 2) return Seq.empty[Int]
      ...
      // Если осталось число больше 1, то оно простое
      if (n > 1) factors += n

      // Приводим к неизменяемой коллекции (List) и сортируем
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)]): Seq[String] = {
      ballsArray
        ...
        .toList  // преобразуем к неизменяемой коллекции
        .sortBy(_._2)
        .map(_._1)
    }

  // Пример коллекции шариков
  ...
  // Версия функции с коллекцией по умолчанию
  def sortByHeavyweightDefault: Seq[String] = sortByHeavyweight(balls)
}