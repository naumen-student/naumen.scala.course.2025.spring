...
object Task1 extends App {
  ...
  object ShowInstance {
    implicit val catShow: Show[Cat] = {
      case c: VeryLittleCat => "очень маленький кот " + c.name
      case c: LittleCat => "маленький кот " + c.name
      case c: NormalCat => "кот " + c.name
      case c: BigCat => "большой кот " + c.name
      case c: VeryBigCat => "очень большой кот " + c.name
    }

    implicit def boxShow[A: Show](implicit Show: Show[A]): Show[Box[A]] = {
      case EmptyBox => "пустая коробка"
      case BoxWith(c) => Show.show(c) + " в коробке"
    }
  }
