...
object Task3 extends App {
  ...
  def countWords(lines: Vector[String]): WordsCount = {
    // Разбиваем строки на слова, подсчитываем каждое слово в строке
    val result = Await.result(
      mapReduce(lines) { line =>
        val words = line.split("\\s+").toVector.filter(_.nonEmpty)
        val counts = words.groupBy(identity).map { case (word, occurrences) => 
          Count(word, occurrences.size)
        }.toSeq
