...
object Task4 extends App {
  ...
  object EIO {
    ...
    implicit def monad[E]: MonadError[EIO, E] = new MonadError[EIO, E] {
      ...
      def flatMap[A, B](fa: EIO[E, A])(f: A => EIO[E, B]): EIO[E, B] = fa match {
        case EIO(Left(e)) => EIO(Left(e))
        case EIO(Right(a)) => f(a)
      }

      def raiseError[A](error: E): EIO[E, A] = EIO(Left(error))

      def handleError[A](fa: EIO[E, A])(handle: E => A): EIO[E, A] = fa match {
        case EIO(Left(e)) => EIO(Right(handle(e)))
        case _ => fa
      }
    }
  }
