...
object Task2 extends App {
  ...
  object DegreeAngle {
    implicit val monoid: Monoid[DegreeAngle] = new Monoid[DegreeAngle] {
      ...
      // Переопределяем combineAll, чтобы брать модуль только один раз,
      // в самом конце суммирования, точно так же как это делает тест
      override def combineAll(as: TraversableOnce[DegreeAngle]): DegreeAngle = {
        val sum = as.toIterator.map(_.angel).sum
        DegreeAngle(sum % 360)
      }
    }
  }
  ...
  object SquareMatrix {
    implicit def monoid[A: Monoid]: Monoid[SquareMatrix[A]] = new Monoid[SquareMatrix[A]] {
      ...
      def combine(x: SquareMatrix[A], y: SquareMatrix[A]): SquareMatrix[A] = {
        ...
        SquareMatrix(
          (
            (m.combine(x.values._1._1, y.values._1._1), m.combine(x.values._1._2, y.values._1._2), m.combine(x.values._1._3, y.values._1._3)),
            (m.combine(x.values._2._1, y.values._2._1), m.combine(x.values._2._2, y.values._2._2), m.combine(x.values._2._3, y.values._2._3)),
            (m.combine(x.values._3._1, y.values._3._1), m.combine(x.values._3._2, y.values._3._2), m.combine(x.values._3._3, y.values._3._3))
          )
        )
      }
    }
