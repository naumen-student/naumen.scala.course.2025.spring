...
object Exercises {
    ...
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
        ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size))
          .map { pic =>
            pic.lines.map(_.map(color => Integer.toUnsignedString(color.getRGB)).mkString(" ")).mkString("\n")
          }
    ...
    def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] = {
        val generateColor = ZIO.serviceWithZIO[ColorService](_.generateRandomColor())
          .mapError(_ => new GenerationError("Не удалось создать цвет"))
        
        val generatePicture = ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size))
          .mapError(_ => new GenerationError("Ошибка генерации изображения"))
        
        for {
            color <- generateColor
            picture <- generatePicture
            result <- ZIO.serviceWithZIO[PictureGenerationService](_.fillPicture(picture, color))
