...
object Exercises {
    ...
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
        ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size))
            .map(picture => picture.lines.map(_.map(color => Math.abs(color.getRGB)).mkString(" ")).mkString("\n"))
    ...
    def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] =
        for {
            color <- ZIO.serviceWithZIO[ColorService](_.generateRandomColor()).mapError(_ => new GenerationError("Не удалось создать цвет"))
            picture <- ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size)).mapError(_ => new GenerationError("Ошибка генерации изображения"))
            filledPicture <- ZIO.serviceWithZIO[PictureGenerationService](_.fillPicture(picture, color)).mapError(_ => new GenerationError("Возникли проблемы при заливке изображения"))
        } yield filledPicture
    ...
    def task4(size: (Int, Int)): IO[GenerationError, Picture] =
        task3(size).provideLayer(utils.ColorService.live >+> utils.PictureGenerationService.live)
