...
object Exercises {
    ...
    def task1(r: Int, g: Int, b: Int): URIO[ColorService, Option[Color]] =
        ZIO.serviceWithZIO[ColorService](_.getColor(r, g, b).map(Some(_)).catchAll(_ => ZIO.succeed(None)))
    ...
    def task4(size: (Int, Int)): IO[GenerationError, Picture] = {
        val colorLayer = ColorService.live
        val combinedLayer = ZLayer.make[ColorService with PictureGenerationService](
            colorLayer,
            PictureGenerationService.live
        )
        task3(size).provideLayer(combinedLayer)
    }
